      ...
      <div class="container-wide">
        ...
        <div v-else class="lg:grid lg:grid-cols-3 lg:gap-8 items-start">
          ...
          <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)] p-6 md:p-10">
              ...
              <div class="flex flex-wrap items-center gap-6 pb-6 border-b border-slate-100">
                <div class="flex items-center gap-2 text-sm text-slate-500">
                  ...
                  <span :class="getTypeStyle(item.type)" class="px-2.5 py-0.5 rounded text-[11px] font-bold border">
                    {{ item.type }}
                  </span>
                </div>
                ...
                <div class="flex items-center gap-2 text-sm text-slate-500">
                  ...
                  <span class="font-medium text-slate-700">{{ item.publishDate }}</span>
                </div>

                <div v-if="item.author" class="flex items-center gap-2 text-sm text-slate-500">
                  <svg class="w-5 h-5 text-hailong-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="flex items-center gap-2 text-sm text-slate-500">
                  ...
                  <span class="font-medium text-slate-700">{{ item.views || 0 }} 次</span>
                </div>
              </div>

              <!-- 摘要 -->
              <div v-if="item.summary" class="mt-6 p-5 bg-slate-50 border-l-4 border-l-hailong-primary rounded-xl">
                <div class="flex items-start gap-3">
                  ...
                  <p class="text-slate-600 text-xs leading-relaxed">{{ item.summary }}</p>
                </div>
              </div>
              
              <!-- 封面图片 -->
              <div v-if="coverImageUrl" class="mt-6 rounded-2xl overflow-hidden border border-slate-100">
                <img :src="coverImageUrl" :alt="item.title" class="w-full h-96 object-cover" />
              </div>

              <!-- 新闻内容 -->
              <div class="mt-8">
                <div class="prose prose-lg max-w-none text-slate-700 leading-relaxed" v-html="item.content"></div>
              </div>
            </div>

            <!-- 标签 -->
            <div v-if="item.tags && item.tags.length > 0" class="bg-white rounded-2xl border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)] p-6">
              <div class="flex items-center gap-3 flex-wrap">
                <span class="text-sm font-bold text-slate-500">标签：</span>
                <span
            ...
            <!-- 附件列表 -->
            <div v-if="item.attachments && item.attachments.length > 0" class="bg-white rounded-2xl border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)] p-6 md:p-8">
          ...
          <div class="lg:col-span-1 space-y-6 lg:sticky lg:top-24 h-fit">
            ...
            <!-- 相关新闻推荐卡片 -->
            <div v-if="relatedItems.length > 0" class="bg-white rounded-2xl border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)] p-6">
              <h3 class="text-sm font-bold text-slate-400 tracking-wider mb-4 uppercase">相关新闻推荐</h3>
              <div class="space-y-4">
                <router-link
