...
###############################################################################
# 第四步：验证唯一的 Compose 配置
###############################################################################
print_step "第四步：验证 docker-compose.yml 配置"

cd "$PROJECT_PATH"
if ! $COMPOSE_CMD --env-file .runtime/secrets.env config --quiet; then
    print_error "docker-compose.yml 配置无效，已停止部署。"
    exit 1
fi

print_info "docker-compose.yml 验证通过。前端会由 nginx/Dockerfile 在镜像构建阶段完成构建。"
