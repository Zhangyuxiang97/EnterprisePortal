...
将公告表的 `province`、`city`、`district` 统一转换为区域字典编码；按已核实的项目系列修正旧清洗程序错误标记为河南省的跨省数据，字典采用“XX市区”汇总节点的城区会归入该编码。其余数据严格按省—市—区层级转换，无法映射或层级关系错误的值会被清空，避免正文中的供应商或候选人地址被误判为项目区域。
